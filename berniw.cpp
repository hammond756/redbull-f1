...
static tdble oldAccel[BOTS];
static tdble oldBrake[BOTS];
static tdble oldSteer[BOTS];
static tdble oldClutch[BOTS];
static tdble prevDist[BOTS];
static tdble distRaced[BOTS];
static int oldFocus[BOTS];//ML
static int oldGear[BOTS];
...
extern "C" int berniw(tModInfo *modInfo)
{
	...
	for (int i = 0; i < BOTS; i++) {
		std::cout << "Initializing all bots...";
...
static void newRace(int index, tCarElt* car, tSituation *situation)
{
	...
	filename << curTrack->category << "_"
					 ...
					 << situation->raceInfo.ncars << "cars_"
					 << ltm->tm_mday
					 << ltm->tm_hour
					 << ltm->tm_min
					 << ltm->tm_sec
...
static void drive(int index, tCarElt* car, tSituation *situation)
{
	...
	float meas_angle =  RtTrackSideTgAngleL(&(car->_trkPos)) - car->_yaw;
	NORM_PI_PI(meas_angle); // normalize the angle between -PI and + PI
